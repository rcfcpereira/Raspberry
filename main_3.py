...
if __name__ == "__main__":
    ...
    try:
        ...
        # Start Message
        lcd_race.draw.text((75, 5), 'Drag Race', fill="WHITE", font=Font1)
        lcd_race.rotate_print(lcd_race.image)

        #image=rotate(180)
        lcd_race.ShowImage(lcd_race.image)

        time.sleep(1)

        # Start Race

        lcd_race.draw.text((55, 100), "Press the button", fill="WHITE", font=Font2)
        lcd_race.draw.text((55, 124), "  to start race ", fill="WHITE", font=Font2)
        ...
        while True:
            ...
            if gpio_race.get_start_button_state() == 0:
                ...
                lcd_race.draw.rectangle([(0, 100), (240, 320)], fill="BLACK")
                lcd_race.draw.text((60, 100), " Race Start in: ", fill="WHITE", font=Font2)
                lcd_race.rotate_print(lcd_race.image)

                for x in range(6):

                    lcd_race.draw.rectangle([(95, 124), (195, 224)], fill="BLACK")
                    lcd_race.draw.text((94, 124), "{}".format(str((5 - x))), fill="RED", font=Font4)
                    lcd_race.rotate_print(lcd_race.image)
                    time.sleep(1)
        ...
        lcd_race.draw.rectangle([(0, 100), (240, 320)], fill="BLACK")
        lcd_race.draw.text((15, 100), "TIME:", fill="WHITE", font=load_font.Font2)
       
        while not(gpio_race.racer_left_cross and gpio_race.racer_rigth_cross):

            lcd_race.draw.rectangle([(20, 124), (240, 148)], fill="BLACK")
            lcd_race.draw.text((20, 124), "{:.3f} s".format(clock), fill="RED", font=Font2)
        ...
        lcd_race.draw.rectangle([(0, 100), (240, 320)], fill="BLACK")
        lcd_race.draw.text((15, 100), "Race Time Left:", fill="WHITE", font=Font2)
        if gpio_race.racer_left == False:
            lcd_race.draw.text((20, 124), "{:.3f} s".format(time_stamp_left), fill="RED", font=Font2)
        else:
            lcd_race.draw.text((20, 124), "False Start", fill="RED", font=Font2)
        lcd_race.draw.text((15, 200), "Race time Rigth:", fill="WHITE", font=Font2)
        if gpio_race.racer_rigth == False:
            lcd_race.draw.text((20, 224), "{:.3f} s".format(time_stamp_rigth), fill="RED", font=Font2)
        else:
            lcd_race.draw.text((20, 224), "False Start", fill="RED", font=Font2)

        lcd_race.rotate_print(image)
        time.sleep(30)

        lcd_race.draw.rectangle([(0, 100), (240, 320)], fill="BLACK")
        lcd_race.draw.text((65, 100), 'END RACE', fill="WHITE", font=Font2)
        lcd_race.rotate_print(image)
        time.sleep(5)
